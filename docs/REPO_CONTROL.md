...
- Active matches resume from local storage with no account, because free play requires no account. Completed matches are written to Neon as of 2026-07-30: `POST /api/matches` records `matches`, `players`, `turns`, and `darts` in one `db.batch`, which the Neon HTTP driver runs as a single transaction, so a match never lands with half its darts missing. `rooms` and `room_members` still have no writer.
