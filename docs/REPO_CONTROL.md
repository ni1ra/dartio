...
- **`matches.state_version` is also the turn number.** Every accepted visit increments it and takes its value, so nothing that does not append a visit may increment it — `completeRoomMatch` sets status and winner only. It is idempotent because both clients replay the same log, see the same finish, and both report it.
...
- A room lives 12 hours, its code avoids O/0 and I/1, and an expired room answers identically to a code that never existed. Opening, joining, and watching a room all require the `online_multiplayer` entitlement; Free carries zero online seats and a gallery chair is an online seat. Playing inside a room shipped in Cycle 15; `PRODUCT_AVAILABILITY.onlineMultiplayer` stays `coming_soon` deliberately until the whole `/friends` promise is true, and retires with host handover in Cycle 24.
...
- `src/lib/voice/dialogue.ts` gives `confirm` and `cancel` something to act on and each mode its own vocabulary: a visit total is X01's alone, and a drill has nobody to pass to. A doubtful transcription is held and answered oldest first. The confidence floor is implemented and tested but `POST /api/voice/transcribe` returns no confidence yet, so nothing reaches the queue in production and nothing in the interface claims it does.
...
- Non-X01 opponents run locally at **levels 1–8 only**, which is exactly the free tier. Levels 9–20 are server-authorized and `POST /api/ai/turn` speaks X01 — extending it would mean teaching the server each mode's rules, which the architecture exists to avoid.
- `use-ai-visit.ts` is why the opponent does not run away with the match. It calls its callback with no arguments deliberately: the callback must read the log from a ref rather than from a closure created a visit earlier, which is the bug that once had X01's AI playing entire matches by itself. Undo and rewind move the ref with the state for the same reason.
...
0d. `node scripts/verify-rooms-live.mjs <preview-url> <preview-db-url>` — the full room round trip: two throwaway identities, one room, a deliberate version collision. **Preview only, and it writes**: it grants temporary Pro rows and deletes them, and refuses to run against production. It exists because the room write was a Postgres syntax error that every unit test passed straight through — a fake database never renders SQL.
