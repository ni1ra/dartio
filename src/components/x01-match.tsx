...
import { AggregateVisitRequiresDartsError, applyAggregateVisit, applyDart, basicCheckoutAdvice, BOARD_CLOCKWISE, BOARD_RADII, chooseAiAim, createX01, dart, notation, representativePoint, scoreBoardPoint, seededRandom, throwAiDart, undoLastDart, x01PlayerStats, type Dart, type InRule, type OutRule, type X01State } from "@/domain";
...
function runLocalAiTurn(state:X01State,level:number):X01State { if(!Number.isInteger(level)||level<1||level>8)throw new Error("Local AI level must be an integer from 1 to 8");let next=state;const rng=seededRandom(state.turns.length*101+level);while(next.status==="playing"&&next.currentPlayer===1){const aim=chooseAiAim(next.scores[1]??501);next=applyDart(next,throwAiDart(level,aim,rng).dart);}return next; }
...
export function X01Match() {
  ...
  function queueLocalAiTurn(next:X01State,localLevel:number){cancelAi();const generation=aiGeneration.current;setMessage(`AI level ${localLevel} is at the oche…`);aiTimer.current=window.setTimeout(()=>{if(generation!==aiGeneration.current)return;const result=runLocalAiTurn(next,localLevel);setGame(result);aiTimer.current=null;setMessage(settledMessage(result));},450)}
  ...
  function undo(){cancelAi();setAiRecovery(null);const previous=undoLastDart(game);if(previous===game)return;commit(previous);setMessage("Latest dart removed");}
  ...
  return <div className="match-page">
    ...
    <CommandDock className="match-dock"><span aria-live="polite">{message}</span><div className={aiRecovery?"ai-access-actions":undefined}>{aiRecovery&&<><span className="ai-access-recovery" role="alert">{aiRecovery.message}</span><button onClick={retryPremiumAi}>{aiRecovery.kind==="denied"?"Check again":"Retry"}</button><button onClick={continueWithLevelEight}>Continue at level 8</button></>}<button onClick={undo} disabled={!game.past.length}>Undo</button><button onClick={openCorrection} disabled={!canCorrectLatestDart}>Correct latest dart</button></div></CommandDock>
    <Modal open={correction} onClose={closeCorrection} title="Correct the latest dart"><div className="correction-body"><p>Undo exactly the latest editable dart, then record that dart again using the board or Each dart input.</p><Button onClick={()=>{undo();setCorrection(false);setInputMode("darts")}} disabled={!canCorrectLatestDart}>Undo latest dart</Button><Button variant="secondary" onClick={closeCorrection}>Keep current score</Button></div></Modal>
  </div>;
}
