...
export function AccountExperience() {
  ...
  return <div className="page-frame account-page account-v2">
    <header className="page-heading account-heading"><div><p className="eyebrow">Your Dartio</p><h1>One identity.<br/><em>Every oche.</em></h1></div><p>Manage the membership attached to your account. Cross-device history and online rooms are still coming soon.</p></header>
    {checkoutSuccess&&<div className="account-success" role="status"><span>✓</span><div><b>Checkout returned successfully.</b><p>{access.status==="ready"&&hasPaidMembership(access.snapshot)?"The signed webhook has confirmed your paid access.":"Stripe is confirming the subscription. Access may take a moment while the signed webhook updates Dartio."}</p><button type="button" onClick={()=>void access.refresh()} disabled={access.refreshing}>{access.refreshing?"Refreshing…":"Refresh membership"}</button></div></div>}
...
function SignedOutAccount(){return <div className="account-signed-out"><Surface className="account-state" tone="raised"><div className="account-avatar guest" aria-hidden="true">↗</div><div><span className="account-state-code">SIGNED OUT</span><h2>Your board still works.<br/>Membership waits for sign-in.</h2><p>Sign in to manage billing and use paid AI, voice, and checkout features. Local scoring never requires an account.</p><div className="account-actions"><Link className="button-link" href="/auth/sign-in">Sign in securely</Link><Link className="button-link button-link-secondary" href="/play">Play locally</Link></div></div></Surface><aside><b>What sign-in unlocks</b><ol><li><span>01</span>Membership and billing control</li><li><span>02</span>Pro AI, voice, and checkout access</li><li><span>03</span>Rooms and cloud history · coming soon</li></ol></aside></div>}
