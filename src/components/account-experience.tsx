...
export function AccountExperience() {
  ...
  return <div className="page-frame account-page account-v2">
    <header className="page-heading account-heading"><div><p className="eyebrow">Your Dartio</p><h1>One identity.<br/><em>Every oche.</em></h1></div><p>Your membership, your record, and the matches behind it. Online rooms are live with Pro.</p></header>
...
function MembershipStatus({access}:{access:ReturnType<typeof useAccess>}){if(access.status==="loading")return <p className="membership-status" role="status">Checking membership…</p>;if(access.status==="unavailable")return <div className="membership-status unavailable"><p>Paid access could not be verified. This does not mean your plan changed.</p><button type="button" onClick={()=>void access.retry()}>Retry access</button></div>;const {snapshot}=access,features=[isProductAvailable(snapshot,"advancedAi")&&hasAccessEntitlement(snapshot,"advanced_ai")?"AI levels 1–20":null,isProductAvailable(snapshot,"advancedCheckout")&&hasAccessEntitlement(snapshot,"advanced_checkout")?"Advanced checkout":null,isProductAvailable(snapshot,"voiceInput")&&hasAccessEntitlement(snapshot,"voice_always_on")?"Voice scoring":null,isProductAvailable(snapshot,"deepStats")&&hasAccessEntitlement(snapshot,"deep_stats")?"Deep statistics":null,isProductAvailable(snapshot,"onlineMultiplayer")&&hasAccessEntitlement(snapshot,"online_multiplayer")?"Online rooms":null,isProductAvailable(snapshot,"customPractice")&&hasAccessEntitlement(snapshot,"custom_practice")?"Custom practice":null].filter(Boolean);const ends=snapshot.accessEndsAt?new Intl.DateTimeFormat(undefined,{dateStyle:"medium"}).format(new Date(snapshot.accessEndsAt)):null;return <div className="membership-status"><p><strong>{snapshot.effectivePlan.toUpperCase()}</strong> · {snapshot.accessState==="grace"?"grace access":snapshot.accessState}</p>{features.length>0&&<p>{features.join(" · ")}</p>}{snapshot.cancelAtPeriodEnd&&<p>Cancellation scheduled{ends?` · access through ${ends}`:""}.</p>}{snapshot.accessState==="grace"&&ends&&<p>Paid access is available through {ends} while billing needs attention.</p>}<small>Club management is still being built.</small></div>}
