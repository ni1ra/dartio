...
const modes: ReadonlyArray<readonly [string,string,string,string,string]>=[
  ...
  ["02","Cricket","Own the numbers, close the board","Standard or cut-throat · rounds · points","#"],
...
export default function PracticePage(){return <div className="page-frame catalog-page"><header className="page-heading wide"><p className="eyebrow">Practice room</p><h1>Pick a pressure.<br/><em>Build a weapon.</em></h1><p>Every mode has an input built for the drill—not a generic score box forced into a different game.</p></header><div className="catalog-list">{modes.map(([n,name,desc,meta,href],i)=>{const content=<><span className="catalog-number">{n}</span><div><h2>{name}</h2><p>{desc}</p></div><small>{meta}</small><b>{i===0?"PLAY NOW →":"COMING NEXT"}</b></>;return <Surface key={name} className={`catalog-row ${i===0?"featured":""}`}>{i===0?<Link href={href}>{content}</Link>:<div className="catalog-static" aria-disabled="true">{content}</div>}</Surface>})}</div></div>}
